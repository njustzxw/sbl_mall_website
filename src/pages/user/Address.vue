...
<div>
	...
	<div class="fn-clear" v-if="addressList.length">
		...
		<div class="address-list-item" v-for="(item,index) in addressList" :key="index">
			<div class="name">
				<div class="name-cell">{{item.receiverName}}</div>
			</div>
			<div class="detail">
				<div class="detail-cell">{{item.receiverProvince+item.receiverCity+item.receiverDistrict+item.receiverAddress}}</div>
			</div>
			<div class="telephone">{{item.receiverMobile}}</div>
			<div class="default" v-if="item.status">
				<span>默认地址</span>
			</div>
			<div class="common" v-else @click="_setDefualtAdress(item.id)">
				<span>设为默认地址</span>
			</div>
			<div class="operation-cell">
				<span @click="mod(item)"><Icon type="edit" size="small" style="color:#5788e6"></Icon></span>
				<span @click="delConfirm(item.id)"><Icon type="close" size="small" style="color:#fb5858"></Icon></span>
			</div>
		</div>
	</div>
...
import {getaddressList,addreciveAddress,setDefualtAdress,deleteAddress,updatereciveAddress} from '@/api/shipping.js'
export default {
	name: "Address",
	props: ["headtitle"],
	mounted () {
